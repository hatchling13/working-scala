...
object Main extends ZIOAppDefault {
  override def run =
    for {
      ...
      b = Board(
        List(
          Email("aa@gmail.com", "이메일제목임", "내용임"),
          SMS("김사장", "메롱"),
          SMS("01012345678", "내용임"),
          VoiceRecording("김사장", "출근해")
        )
      )
      _ = println(b)
      _ = b.notifications.foreach { noti =>
        ...
        val a = noti match {
          case Email(sourceEmail, title, body) =>
            s"이 이메일은 영국에서 시작되었으며 ... $title"
          case SMS(sourceNumber, message) if sourceNumber == "김사장" =>
            "차단" // 패턴 가드
          case SMS(sourceNumber, message) => s"국제번호에서 온 SMS입니다."
          case VoiceRecording(contactName, link) =>
            s"아쉽지만 text로 들려줄 수 없어 $contactName 으로 전화하세요"
