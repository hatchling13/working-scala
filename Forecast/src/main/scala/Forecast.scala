...
object DiscordWebhook extends ZIOAppDefault {
  ...
  def parseItem(
      item: ujson.Value,
      regionMap: Map[(String, String), String]
  ): String = {
    val category = getKeyFromJson(item, "category")
      .map(getWeatherStatus)
      .getOrElse("Unknown Category")
    val fcstDate =
      getKeyFromJson(item, "fcstDate").map(formatDate).getOrElse("Empty Date")
    val fcstTime =
      getKeyFromJson(item, "fcstTime").map(formatTime).getOrElse("Empty Time")
    ...
    val region =
      regionMap.getOrElse((nx.toString(), ny.toString()), "Unknown region")
  ...
  def getWeatherInfo(json: Value, regionMap: Map[(String, String), String]) =
    for {
      response <- ZIO
        .fromOption(json.obj.get("response"))
        .mapError(_ => "response not found in json")
      body <- ZIO
        .fromOption(response.obj.get("body"))
        .mapError(_ => "body not found in json")
      items <- ZIO
        .fromOption(body.obj.get("items"))
        .mapError(_ => "items not found in json")

      itemArray <- ZIO
        .fromOption(items.obj.get("item").collect { case ujson.Arr(itemArray) =>
          itemArray
        })
      ...
      .take(5)
      .map(item => parseItem(item, regionMap))
      .mkString("\n")
  ...
  def getRainStatus(fcstValue: String): String =
    if (fcstValue == "0") "강수 없음" else "강수 있음"
  ...
  def getRegionMap: Map[(String, String), String] = {
    ...
    reader
      .collect { case Right(data) =>
        (data.nx, data.ny) -> data.dong
      }
      .toList
      .toMap
  }
...
object RegionData {
  ...
  implicit val headerDecoder: HeaderDecoder[RegionData] =
    HeaderDecoder.decoder("격자 X", "격자 Y", "3단계")(RegionData.apply _)
}
